...
use sea_orm::{DbErr, Schema, ConnectionTrait};
...
pub async fn setup_schema(db: &sea_orm::DatabaseConnection) -> Result<(), DbErr> {
    ...
    let _ = db.execute(sea_orm::Statement::from_string(
        builder,
        "INSERT OR IGNORE INTO \"user_group\" (id, name) VALUES (1, 'Admin Group');".to_string()
    )).await;
...
pub fn create_router(db: sea_orm::DatabaseConnection) -> axum::Router {
    axum::Router::new()
        .route("/", axum::routing::get(|| async { "rsahp backend running" }))
