...
pub fn render_login_modal(ctx: &egui::Context, state: &mut AuthState, api_base_url: &str) {
    ...
    egui::Window::new(if state.is_registering { "Create Account" } else { "Login" })
            ...
            ui.horizontal(|ui| {
                if state.auth_request_in_progress {
                    ...
                    if ui.button(btn_text).clicked() {
                        let endpoint = if state.is_registering { "/register" } else { "/login" };
                        let url = format!("{}/auth{}", api_base_url.replace("/documents", ""), endpoint); // Very hacky url handling for now
                                ...
                                let res = match result {
                                    Ok(response) => {
                                        if response.status == 200 {
                                            ...
                                            Err(format!("Error {}: {:?}", response.status, response.text()))
                                        }
                                    }
                                    Err(e) => Err(e),
                                };
                    ...
                    let toggle_text = if state.is_registering { "Switch to Login" } else { "Create Account" };
